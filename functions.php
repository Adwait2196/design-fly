...
if ( ! function_exists( 'design_fly_setup' ) ) :
	/**
	 ...
	 */
	function design_fly_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on DesignFly, use a find and replace
		 * to change 'design-fly' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'design-fly', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'design-fly' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'design_fly_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'design_fly_setup' );
 ...
 */
function design_fly_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'design_fly_content_width', 640 );
}
add_action( 'after_setup_theme', 'design_fly_content_width', 0 );
 ...
 */
function design_fly_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'design-fly' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'design-fly' ),
...
add_action( 'widgets_init', 'design_fly_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'design_fly_scripts' );
